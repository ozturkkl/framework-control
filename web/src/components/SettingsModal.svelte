    ...
    <div class="space-y-4">
      ...
      >
        <div class="flex-1 space-y-1">
          <div class="flex items-center gap-2">
            ...
            {:else}
              <div class="badge badge-ghost badge-sm gap-1">
                <Icon icon="mdi:loading animate-spin" class="w-3 h-3" />
                Checking...
              </div>
            {/if}
          </div>
          <div class="text-xs opacity-70 space-y-1 mt-1">
            {#if isPaused}
              ...
              <div>Click "Unpause" to resume update notifications.</div>
            {:else if newVersionAvailable}
              <div>
        ...
        <div class="flex flex-col items-end gap-2 md:w-auto">
          <div class="flex items-center gap-2">
            {#if isPaused}
              <button class="btn btn-sm btn-primary" on:click={unpauseUpdates}>
                Unpause
              </button>
            {:else if newVersionAvailable}
              <button
              ...
              {#if errorMessage}
                <div class="text-xs opacity-70">{errorMessage}</div>
              {/if}
            {/if}
          </div>
      ...
      <section class="flex items-center justify-between gap-4">
        <div>
          <h4 class="font-semibold">Desktop Shortcuts</h4>
          <p class="text-xs opacity-70">
