...
async function main() {
	...
	if (!config.port) {
		throw new Error('FRAMEWORK_CONTROL_PORT is required (via env var or service/.env)');
	}
	...
	if (config.updateRepo === undefined) {
		throw new Error('FRAMEWORK_CONTROL_UPDATE_REPO is required (via env var or service/.env)');
	}
	...
	if (useStaticBuild) {
		console.log('[build-linux] Using x86_64-unknown-linux-musl target for static linking (CI mode)...');
	} else {
		console.log('[build-linux] Using default target for dynamic linking (local dev mode)...');
	}
	...
	const binarySource = targetPath
		? path.resolve(serviceDir, 'target', targetPath, 'release', 'framework-control-service')
		: path.resolve(serviceDir, 'target', 'release', 'framework-control-service');
	...
	fs.writeFileSync(serviceFile, generateMinimalServiceFile(config.port), 'utf8');
	...
	await run('tar', ['-czf', tarballPath, '-C', distDir, 'framework-control', 'framework-control.service']);
