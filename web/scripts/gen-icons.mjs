...
async function generate() {
	...
	for (const out of outputs) {
		...
		await sharp(source).resize(out.size, out.size, { fit: 'cover' }).png().toFile(dest);
	...
	// Copy logo.png to generated for Linux desktop entry embedding
	const pngDest = path.resolve(outDir, 'logo.png');
	await fs.promises.copyFile(source, pngDest);
	console.log(`[gen-icons] Wrote ${path.relative(publicDir, pngDest)}`);
}
